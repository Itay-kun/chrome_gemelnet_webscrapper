doc_id = 'AKfycbwnjeuB8mQKLXm9opklyTUhHssD7LKf75MAB8TsK63TxM4CAI4m89nMYih4qB_vd54_rQ';
...
function buildGemelnetURL(id) {
    let link = 'https://gemelnet.cma.gov.il/views/perutHodshi.aspx?idGuf=' + id.toString() + '&OCHLUSIYA=1';
  return link;
}

function buildGemelnetDataURL(resource_id="a30dcbea-a1d2-482c-ae29-8f781f5025fb",guf_id=getID()){
    let link = "https://data.gov.il/api/3/action/datastore_search?resource_id="+resource_id+"&q="+guf_id    
return link
}
...
function openGufsPages(ids = []) {
  idsSet = [...new Set(ids)];
  idsSet.map(id => {open(buildGemelnetURL(id))});
}
/****************************************************************************************/
...
function getTables(page = window) {
  let all_tabels =
    page.document.all.VisibleReportContentReportViewer1_ctl10.getElementsByTagName('table');
  let tables_set = new Set();
  for (i = 0; i < all_tabels.length; i++) {
    current_table = getTableContent(i);
    if (all_tabels[i].innerText.includes('באחוזים') && current_table.length < 98)
    {
